number = ""
highest_product = 0
true = 1

for a in range(0,len(number),2):
    product = 1
    group = number[a:a+8]
    if len(group) < 8:
        pass
    else:
        for i in range(4):
            product *= int(group[2*i:2*i+2])
        if product > highest_product:
            highest_group = group
            highest_product = product



    group = ""
    product = 1
    for b in range(4):
        group += number[a+b*20*2:a+b*20*2 + 2]
    if len(group) < 8:
        pass
    else:
        for i in range(4):
            product *= int(group[2*i:2*i+2])
        if product > highest_product:
            highest_group = group
            highest_product = product



    group = ""
    product = 1
    for b in range(4):
        group += number[a+b*21*2:a+b*21*2 + 2]
    if len(group) < 8:
        pass
    else:
        for i in range(4):
            product *= int(group[2*i:2*i+2])
        if product > highest_product:
            highest_group = group
            highest_product = product

    group = ""
    product = 1
    for b in range(4):
        group += number[a+b*19*2:a+b*19*2 + 2]
    if len(group) < 8:
        pass
    else:
        for i in range(4):
            product *= int(group[2*i:2*i+2])
        if product > highest_product:
            highest_group = group
            highest_product = product


print(highest_product,highest_group)